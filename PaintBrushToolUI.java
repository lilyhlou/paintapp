...
public class PaintBrushToolUI extends JPanel implements ChangeListener {
	...
	public PaintBrushToolUI() {
		...
		brush_size = new JSlider(0, 40, 5);
		brush_size.addChangeListener(this);
		brush_size_panel.add(brush_size, BorderLayout.CENTER);

		JPanel brush_opacity_panel = new JPanel();
		JLabel brush_opacity_label = new JLabel("Opacity: ");
		brush_opacity_panel.setLayout(new BorderLayout());
		brush_opacity_panel.add(brush_opacity_label, BorderLayout.WEST);
		brush_opacity = new JSlider(0, 100, 100);
		brush_opacity.addChangeListener(this);
		brush_opacity_panel.add(brush_opacity, BorderLayout.CENTER);
		...
		slider_panel.add(brush_opacity_panel);
	...
	public double getOpacity() {
		return (double) brush_opacity.getValue() / 100.0;
	}

	public void setBrush(Pixel p) {
		ObservablePicture preview_frame = color_preview.getPicture();
		preview_frame.suspendObservable();
		for (int x = 0; x < preview_frame.getWidth(); x++) {
			for (int y = 0; y < preview_frame.getHeight(); y++) {
